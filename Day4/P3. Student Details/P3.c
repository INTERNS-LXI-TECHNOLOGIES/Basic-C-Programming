/*
Program to read 5 Students details and perform following functions using Structure Array & Functions
a.	Search
b.	Total Mark
c.	Percentage
d.	Display All.
...
void Search()
{
    ...
    for(int i=0;i<5;i++)
    {
        if(a==s[i].rNo)
        {
            printf("\n \n Roll Number : %d \n Name : %s \n Age : %d \n Mark in Subject 1 : %d \n Mark in Subject 2 : %d \n Mark in Subject 3 : %d \n ",s[i].rNo,s[i].name,s[i].age,s[i].m[1],s[i].m[2],s[i].m[3]) ;
            return ;
        }
    }
        printf("\n Invalid Roll Number.") ;
...
void Display()
{
   ...
   {
       printf("\n Details of Student %d : \n \n Name : %s \n Roll Number : %d \n Age : %d",i+1,s[i].name,s[i].rNo,s[i].age) ;
